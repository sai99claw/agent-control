...
產品負責人 / 主線 / 開題者 / Worker(實作者)/ 驗證者 / 落地器(程式)/ 知識維護。角色是責任分類,不等於一排常駐模型;作者與審查者應使用分離的執行上下文。

**v1.1 那份提案裡的「協調者 / 規劃者 / Git 整合管理者」已合併或退場**(2026-09-21,D-010):排程與派工的判斷回主線,機械的部分是 `scripts/land.sh`;調度員這個角色整個拿掉。
...
## 設計文件的固定段(`docs/DESIGN-<題>.md`;2026-09-23,#29 A1)

設計 session(`memory/role/design.md`)交的就是這一份。**段落是固定的**,不是每次重想一次 ——
少了哪一段,下一個人分不出「這一題沒有那個面向」與「上一個人忘了寫」(D-015 §5.5 的母題)。

| 段 | 寫什麼 | 為什麼非有不可 |
|---|---|---|
| ① 起因 | 使用者原話(逐字)、量到的症狀、每句標**實測 / 讀 code 推的** | 轉述過的原話會在第三手變成另一個題目 |
| ② 結論表 | 一列一個取捨:`結論` / `理由` / **`未來每票省/多花`**(D-018,粗估標推的) | 沒有這一欄的設計文件,是照「現在改起來多難」決定的 |
| ③ 否決案 | 甲乙丙各一段:方案、為什麼不選 | 只留結論的文件,下一個人會把同一條路再走一次 |
| ④ 遷移 | 既有資料 / 既有檔案怎麼過去;過渡期兩種形狀並存多久 | 「改完就好」是設計文件最常見的漏洞 |
| ⑤ 驗收表 | 每條結論對到一條**會紅的斷言或可 grep 的文件修改** | 驗收寫不出來的結論,是還沒設計完 |
| ⑥ 後續工作的模型表 | 一列一件工作:`做什麼` / `票號(未開就寫「未開」)` / `模型(fable/opus/sonnet/haiku)` / `一行理由` | 主線拿它直接派工,不必再判一次 |

**不准用「改個名 / 加第二個欄位 / 兩邊各做各的形狀」繞過去**(D-018);太複雜可以裁
「開票之後再做」,但②要寫出為什麼現在不做、⑥要有那一列。

## 20. 必須通過的代表性情境
兩張無衝突票平行、依賴票不誤啟動;越界寫入被攔;主線改變後舊結果失效;重啟後恢復且無重複合併;交接包足以接手;原生記憶與決策衝突被記錄;取消後遲到回報不能改 Done;無 token 資料顯示未知;驗證者寫的案例在乾淨主線上真的會紅;看板決策後主線看到同一結果。
