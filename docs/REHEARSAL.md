...
| `scripts/gate.sh` 的「環境可疑就不自動派」(D-019) | 下一次真閘門紅而那一輪的 `environment_suspect` 非空(連紅達門檻的 rc=86,或 log 上真的出現一行 `ENVIRONMENT-SUSPECT:`) | stdout 有一行「環境可疑,不自動派」並印出筆數,而**那一輪一個 worker 都沒被起**(`reports/t<票號>/<run_id>/` 底下沒有新的 `EVIDENCE-round…`);收件匣走環境那一頁,`引擎:` / `來源:` / `同形訊息:` 三行都有值。**「筆數印 `?`」是第三種答案** —— 那是狀態檔讀不到、被當成有,不是真的量到 0 |
| `scripts/status.py run-tests` 把案例 stdout 導進 `--log`(D-019) | 下一次有案例真的自己印一行 `ENVIRONMENT-SUSPECT:` 然後把自己記成 skip | 那一行**在 `--log` 那份檔裡**(不是只在呼叫者的終端機上),而同一輪的 `environment_suspect` 有一筆 `source=declared`、`engine` / `why` / `line` 對得上那一行。**log 裡沒有那一行時,這一格會是空的而且一句話都不說** —— 那與「這一趟環境沒問題」長得一樣 |
