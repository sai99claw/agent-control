...
2. 高階模型跟該模型(同 session 或下一個 session)**討論**:哪些要合併、哪些降級成「查得到就好」(搬去 `code-map/` 或 `docs/`,留一行指路)、哪些是反例必須留、哪些已經過期。這一步像老師帶學生:不是替它刪,是幫它分辨。
3. 討論的產出兩種,都要寫回檔案:
...
5. 事件 `memory.consolidated` 記錄整理者、前後大小、上限有沒有變。
