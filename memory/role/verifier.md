...
② 拿到指定的 patch 之後 —— 跑 `python3 scripts/verify-case.py check <票號>`(它自己做乾淨主線副本、覆上同一份案例、兩邊各跑一次,把案例數、紅的是哪幾條、skip、兩邊 sha 寫進票的 `verify.baseline`);交付物用 `python3 scripts/verify-case.py extract <票號>` 出;`EVIDENCE-verifier.md` **檔尾再加一塊 `result`**(2026-09-22):一段 `## result`,底下一塊語言標記是 `result` 的 fenced JSON(鍵見 `tickets/SCHEMA.md`),**寫不出來的欄位照實留空,不要編**。然後**結束**。
