...
| 依賴與衝突 | `depends_on` (list of {id, condition}), **`allowed_write_paths`** (list of globs), `shared_resources` | ✓ | 排程器與落地器都讀 |
| 執行 | `role`, `model`, `tool`, **`attempt`** (int), `attempt_history` | ✓ | 每次派工 attempt+1;遲到的回報對不上 attempt 就拒絕 |
| 版本 | **`base_sha`**, `branch`, `workspace` | ✓ | 閘門結果只對 base_sha 有效 |
| 控制 | `state`, **`state_version`**, `lease` ({holder, until}), `retry_limit`, `budget` | ✓ | state 見 `docs/WORKFLOW.md`;state_version 每次變更 +1 |
| 交付 | `patch_ref`, `review_ref`, `test_evidence` (list), `handoff`, `knowledge_updates` | Done 前 | evidence 每筆帶 base_sha、指令、rc、log 路徑 |
